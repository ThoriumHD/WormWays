...
const ServerSnake_1 = require("./ServerSnake");
class GameWorld {
    players = new Map();
    playerInputs = new Map();
    /** Track last processed client tick for each player */
    lastProcessedClientTick = new Map();
    /** Track pending collisions for 1-tick confirmation window (border collisions are instant) */
    pendingCollisions = new Map(); // collision key -> confirmation state
    food = [];
    tick = 0;
    lastTickTime = 0;
    tickRate = 30; // 30 FPS
    tickInterval = 1000 / this.tickRate;
    // World configuration
    WORLD_SIZE = 4000; // Radius of circular world - match client's visible red circle
    ...
    addPlayer(playerId, name) {
        ...
        const snake = new ServerSnake_1.ServerSnake(playerId, name);
    ...
    spawnFood(isInitialSpawn = false) {
    ...
    addBoostTrailPellet(x, y, radius, snakeColor = '#4a90e2') {
    ...
    addDeathDotsAsFood(snakePoints, snakeRadius, snakeColor = '#4A90E2') {
     ...
     */
    determineCollisionDirection(snake1, snake2) {
     ...
     */
    getSnakeVelocity(snake) {
        // Get current speed and angle
        const speed = snake.boosting ? ServerSnake_1.SNAKE.boostSpeed : ServerSnake_1.SNAKE.speed;
     ...
     */
    checkFoodCollisionWithSnake(food, snake) {
        ...
        for (let i = 0; i < frontThirdCount; i++) {
            ...
            const distance = Math.sqrt((segment.x - food.x) ** 2 + (segment.y - food.y) ** 2);
        ...
        for (const food of this.food) {
            ...
            if (food.committedSnakeId && this.players.has(food.committedSnakeId)) {
                ...
                if (committedSnake && !committedSnake.isDead) {
                    const committedDistance = Math.sqrt((committedSnake.head.x - food.x) ** 2 + (committedSnake.head.y - food.y) ** 2);
    ...
    checkCollisions() {
        for (const [playerId, snake] of this.players) {
            ...
            for (const [otherPlayerId, otherSnake] of this.players) {
                if (otherPlayerId === playerId || otherSnake.isDead)
                    continue;
                ...
                for (let i = 1; i < otherPoints.length; i++) {
                    ...
                    const segmentDistance = Math.sqrt((head.x - segment.x) ** 2 + (head.y - segment.y) ** 2);
