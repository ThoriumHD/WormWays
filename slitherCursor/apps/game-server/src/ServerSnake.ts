// apps/game-server/src/ServerSnake.ts
...
export class ServerSnake {
  ...
  private _currentSpeed = SNAKE.speed; // smooth speed transition

  // Boost trail system
  private boostTrailDots: Array<{
    x: number;
    y: number;
    radius: number;
    age: number; // Age in seconds
    maxAge: number; // How long the dot lasts
    processed: boolean; // Whether this dot has been added to food system
  }> = [];
  ...
  addFoodPoints(points: number) {
    ...
    const segmentsToAdd = Math.floor(this.fractionalFoodPoints / SNAKE.pointsPerSegment);
    
    if (segmentsToAdd > 0) {
      this.actualSegments += segmentsToAdd;
      this.fractionalFoodPoints -= segmentsToAdd * SNAKE.pointsPerSegment; // Keep remainder
    }
  }
  ...
  get effectiveTurnRate() {
    ...
    const reduction = SNAKE.turnRateReduction * lengthFactor;
    return SNAKE.turnRate * (1.0 - reduction);
  }

  reset(pos: Vec2, angle = 0) {
    this.angle = angle;
    this._currentSpeed = SNAKE.speed;
  ...
  step(dt: number, desiredAngle: number) {
    ...
    const angDiff = (target: number, current: number): number => {
    ...
    if (da > maxTurnAngle) da = maxTurnAngle;
    if (da < -maxTurnAngle) da = -maxTurnAngle;
  ...
  private updateBoostTrail(dt: number) {
    ...
    if (this.boosting && this.canBoost) {
      ...
      this.actualSegments = Math.max(SNAKE.minSegments, this.actualSegments);
    }
  }
  ...
  private ensureCorrectSegmentCount() {
    ...
    if (needCount < 1) {
      ...
      this.resampled.length = SNAKE.minSegments;
      return;
    }
  ...
  triggerDeath() {
    ...
    for (let i = 0; i < pelletCount; i++) {
      ...
      this.deathDots.push({
        ...
        radius: SNAKE.boostTrailRadius * 3, // Double size
        progress: 0,
  ...
  updateDeathAnimation(dt: number) {
    if (!this.isDead) return;
