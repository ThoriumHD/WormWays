import { SNAKE } from '../snake/SnakeBody';
...
export class Camera {
  ...
  update(dt: number, target: Vec2, speed: number = 0, growthFactor: number = 1.0) {
    ...
    // Adjust zoom based on speed (zoom out when boosting) and snake growth (reduced zoom compensation)
    const speedZoom = 1.0 + Math.min(0.2, speed / 1000); // max 20% zoom out for speed
    const growthZoom = 1.0 / Math.pow(growthFactor, SNAKE.cameraZoomCompensation);
    this.targetZoom = speedZoom * growthZoom;
